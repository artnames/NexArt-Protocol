        ...
        <article className="prose-protocol prose-spec">
          ...
          <nav className="mb-10 pb-6 border-b border-border">
            ...
            <ul className="flex flex-wrap gap-x-5 gap-y-2 text-sm list-none pl-0">
              ...
              <li><a href="#build-snapshot" className="text-body underline underline-offset-2 hover:text-foreground">Build CER Bundle</a></li>
              ...
              <li><a href="#verification-outcomes" className="text-body underline underline-offset-2 hover:text-foreground">Reason Codes</a></li>
          ...
          <section id="minimum-integration">
            <div className="bg-muted/50 border border-border rounded-md p-5 my-6">
              ...
              <ol className="text-sm text-body space-y-2 pl-5 mb-0">
                <li><strong>Build CER bundle</strong> — Render via CLI or API, receive a sealed CER bundle JSON with hashes.</li>
                <li><strong>Attest to node</strong> — Submit the CER bundle JSON to a canonical attestation node for a signed receipt.</li>
                <li><strong>Store bundle + proof</strong> — Persist the CER bundle JSON and signed receipt for audit.</li>
              </ol>
            </div>
          </section>
          ...
          <section id="build-snapshot">
            ...
            <p>
              Render your code through the CLI. The output includes a CER bundle JSON file (<code>out.snapshot.json</code>) that
              binds your code, seed, VARs, and rendered output together with SHA-256 hashes.
            </p>

            <pre className="spec-code"><code>{`# Render and generate CER bundle JSON
...
# Output: [nexart] Result: PASS`}</code></pre>

            <p>
              The CER bundle JSON (<code>out.snapshot.json</code>) contains the <code>certificateHash</code> — a SHA-256 digest over the canonical JSON of the
          ...
          <section id="attest-to-node">
            ...
            <p>
              Submit the sealed CER bundle JSON to a canonical attestation node. The node verifies internal hash consistency and
              returns a signed receipt proving the bundle was recorded in the proof ledger.
            </p>

            <pre className="spec-code"><code>{`# Attest the CER bundle JSON to the canonical node
npx --yes @nexart/cli@0.2.3 attest out.snapshot.json \\
  --nodeUrl https://nexart-canonical-renderer-production.up.railway.app \\
  --apiKey $NEXART_API_KEY`}</code></pre>
  ...
  "kid": "<attestorKeyId from node's .well-known/nexart-node.json>"
            ...
            <p>
              <strong>Important:</strong> Attestation verifies the bundle's hashes and returns a signed receipt.
              It does not re-run your code, re-render the output, or alter the recorded CER bundle in any way.
            </p>
          </section>
          ...
          <section id="store-bundle">
            ...
            <pre className="spec-code"><code>{`// The CER bundle JSON contains everything needed for offline verification:
            ...
            <div className="overflow-x-auto my-6">
              <table className="spec-table">
                ...
                <tbody>
                  ...
                  <tr><td><code>snapshot.seed</code></td><td>Always</td><td>No</td><td>Required for deterministic replay</td></tr>
                  <tr><td><code>snapshot.vars</code></td><td>Always</td><td>No</td><td>Input parameters (VAR[0..9])</td></tr>
                  <tr><td><code>snapshot.protocolVersion</code></td><td>Always</td><td>No</td><td>Protocol version used for render</td></tr>
                  ...
                  <tr><td><code>signature</code></td><td>Recommended</td><td>No</td><td>Ed25519 signature over receipt</td></tr>
                  <tr><td><code>attestorKeyId</code></td><td>Recommended</td><td>No</td><td>Key ID for signature verification</td></tr>
                  <tr><td><code>nodeRuntimeHash</code></td><td>Optional</td><td>Yes</td><td>Legacy attestation field</td></tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="overflow-x-auto my-6">
              <table className="spec-table">
                ...
                <tbody>
                  ...
                  <tr><td><code>CODE_HASH_MISMATCH</code></td><td>The source code doesn't match its recorded hash.</td></tr>
          ...
          <section id="try-it">
            ...
            <div className="bg-muted/50 border border-border rounded-md p-5 my-6">
              ...
              <ol className="text-sm text-body space-y-2 pl-5 mb-4">
                <li>Run:
                  <pre className="spec-code mt-2 mb-0"><code>{`npx --yes @nexart/cli@0.2.3 run ./examples/sketch.js \\
  --seed 42 --include-code --out demo.png`}</code></pre>
                </li>
                <li>Open <code>demo.snapshot.json</code> — inspect the <code>certificateHash</code></li>
                <li>Verify on <a href="https://recanon.xyz" target="_blank" rel="noopener noreferrer" className="text-body underline underline-offset-2 hover:text-foreground">Recânon</a> — upload the CER bundle JSON to audit integrity</li>
              </ol>
            </div>
          </section>
